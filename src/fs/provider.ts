  ...
  async readFile(uri: vscode.Uri): Promise<Uint8Array> {
    try {
      ...
      if (
        ...
        metadata.contentLength > config.maxPreviewSizeBytes
      ) {
        const sizeInMB = (metadata.contentLength / (1024 * 1024)).toFixed(1);
        const maxSizeInMB = (
          config.maxPreviewSizeBytes /
          (1024 * 1024)
        ).toFixed(1);

        const choice = await vscode.window.showWarningMessage(
          `File is ${sizeInMB}MB, which exceeds the ${maxSizeInMB}MB limit. Opening large files may be slow.`,
          "Open Anyway",
          "Cancel"
        );
